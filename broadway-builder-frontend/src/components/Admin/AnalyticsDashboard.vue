<template>
  <div>
    ...
    <v-layout align-center align-content-center row>
      <v-flex xs6>
        <v-select @change="updateCharts()" :items="months" label="Month" v-model="month"></v-select>
      </v-flex>
      <v-flex xs6>
        <v-select @change="updateCharts()" :items="years" label="Year" v-model="year"></v-select>
      </v-flex>
    </v-layout>
...
export default {
  ...
  methods: {
    ...
    getSuccessfulLoginData() {
      axios
        .get(
          `https://api.broadwaybuilder.xyz/api/useranalytics/numberofsuccessfullogins/${this.month}/${this.year}`
        )
        .then(response => {
          this.successfulLoginData = [
            ["Month and Year", "Min", "Avg", "Max"],
            [
              this.formatMonthAndYear(this.month, this.year),
              response.data.MinSuccessfulLogins,
              response.data.AverageSuccessfulLogins,
              response.data.MaxSuccessfulLogins
            ]
          ];
        });
    },
    getSessionLengthData() {
      axios
        .get(
          `https://api.broadwaybuilder.xyz/api/useranalytics/sessionlength/${this.month}/${this.year}`
        )
        .then(response => {
          this.sessionLengthData = [
            ...
            [
              this.formatMonthAndYear(this.month, this.year),
              response.data.MinSessionLength,
              response.data.AverageSessionLength,
              response.data.MaxSessionLength
            ]
          ];
        });
    },
    getSuccessfulVsFailedLoginData() {
      axios
        .get(
          `https://api.broadwaybuilder.xyz/api/useranalytics/successfulvsfailedlogins/${this.month}/${this.year}`
        )
        .then(response => {
          this.successfulVsFailedLoginData = [
            ...
            [
              this.formatMonthAndYear(this.month, this.year),
              response.data.SuccessfulLogins,
              response.data.FailedLogins
            ]
          ];
        });
    },
